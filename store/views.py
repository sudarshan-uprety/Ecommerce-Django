...
def search(request): #when a search is done this function is called 
    if 'keyword' in request.GET: #here we are checking if the GET request has option keyword or not
        keyword=request.GET['keyword'] #if there is keyword then we are storing it into keyword variable
        if keyword:
    ...
    return render(request,'store/store.html',context) #and this will return the rendered search page
